...
int test_student()
{
	...
	assert(result == 3,
	       "my test 1: day is used 3 times in the list, but got %d.", result )
	       ? passcount++ : failcount++ ;
	...
	assert(strcmp(word.unique_word, "with") == 0, "last word in the list is with, but got %s.", word.unique_word )
	  ? passcount++ : failcount++ ;
